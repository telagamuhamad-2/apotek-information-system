...
<div class="bg-white rounded-xl shadow-sm">
    ...
    <div class="p-6 border-b border-gray-200">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="flex items-center">
                <i class="fas fa-shopping-cart text-2xl text-emerald-600 mr-3"></i>
                <div>
                    <h2 class="text-xl font-semibold text-gray-800">Daftar Penjualan Obat</h2>
        ...
        <!-- Filters -->
        <div class="mt-6">
            <form method="GET" action="{{ route('penjualan.index') }}" class="flex flex-col lg:flex-row gap-4">
                <div class="flex-1">
                    <input type="text"
                           name="search"
                           value="{{ request('search') }}"
                           placeholder="Cari obat atau pelanggan..."
                ...
                <div class="flex gap-2">
                    ...
                    @if(request()->anyFilled(['search', 'product_type_id']))
                        <a href="{{ route('penjualan.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors">
    ...
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($productOutgoings as $sale)
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {{ number_format($sale->product_each_price, 0, ',', '.') }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">
                            {{ number_format($sale->product_total_price, 0, ',', '.') }}
                        </td>
    ...
    <!-- Pagination -->
    @if($productOutgoings->hasPages())
        <div class="p-6 border-t border-gray-200">
            {{ $productOutgoings->appends(request()->query())->links() }}
        </div>
    @endif
</div>
@endsection
